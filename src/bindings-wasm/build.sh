...
PKG_VERSION="$VERSION" node -e "
  ...
  const dist = { name: pkg.name, version: process.env.PKG_VERSION, description: pkg.description,
    publisher: pkg.publisher, author: pkg.author, license: pkg.license, main: 'index.js', types: 'index.d.ts',
    repository: pkg.repository };
  require('fs').writeFileSync('$DIST_DIR/package.json', JSON.stringify(dist, null, 2) + '\n');
"
# ── Format ────────────────────────────────────────────────────────────────────
