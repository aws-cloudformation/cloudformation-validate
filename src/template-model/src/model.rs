...
impl SemanticModel {
    ...
    pub fn parse(bytes: &[u8], config: ParseConfig) -> Result<ParseResult, ParseError> {
        ...
        // Resource and condition entries generated by Fn::ForEach must obey the
        // same shape contracts as directly authored entries, so validate only
        // after language-extension expansion has produced the final section maps.
        let resource_shape_diagnostics =
            crate::parser::resource_shape::validate_resource_attributes(&ir.arena, ir.resources, &ir.span_index);
        ...
        diagnostics.extend(resource_shape_diagnostics);
