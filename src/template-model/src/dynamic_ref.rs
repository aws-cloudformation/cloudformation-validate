//! Dynamic-reference format validation.
//!
...
//!
//! * Only the **last** `{{resolve:...}}` occurrence in a string is validated:
//!   extraction is greedy from the last `{{` to the last `}}`, so a string with
//!   several references is judged by its final one (CloudFormation's own
//!   behavior).
...
//!
//! The check runs over the raw IR arena so both engines surface identical
...
pub fn validate_dynamic_references(arena: &Arena) -> Vec<Diagnostic> {
    let mut out = Vec::new();
    for idx in 0..arena.len() {
        ...
        if let Some(message) = dynamic_reference_error(s) {
...
fn string_is_function_argument(path: &str) -> bool {
    path.split('/').any(|segment| segment.starts_with("Fn::") && segment != "Fn::If")
}
...
mod tests {
    ...
    #[test]
    fn validate_over_arena_reports_e1050() {
        let mut arena = Arena::new();
        arena.alloc(SpannedNode {
            node: Node::String("{{resolve:not-a-service:foo}}".into()),
            span: diagnostics::UNKNOWN_SPAN,
            path: "Resources/R/Properties/X".into(),
        });
        let diags = validate_dynamic_references(&arena);
        assert_eq!(diags.len(), 1);
        assert_eq!(diags[0].rule_id, "E1050");
    }
}
