...
See [rego-engine README](rego-engine/README.md) for the full list. Key ones:

| Builtin | Purpose |
|---------|---------|
| `cfn.resolve(resource_id, path)` | Resolve a property through intrinsics |
| `cfn.has_property(resource_id, path)` | Check if a property exists |
| `cfn.resources_of_type(type)` | Get all logical IDs of a resource type |
| `cfn.is_dynamic(resource_id, path)` | Check if a property is unresolvable |
| `cfn.schema_enum(resource_type, prop)` | Get allowed enum values |
