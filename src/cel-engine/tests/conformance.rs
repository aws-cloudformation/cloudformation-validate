...
"#,
        ...
        assert!(
            !ids.iter().any(|id| id.starts_with("F0")),
            "No structure (fatal) errors expected, got: {:?}",
            ids
        );
    }
...
"#,
        ...
        assert!(
            ids.contains(&"F0002".to_string()),
            "Expected F0002 for bad format version, got: {:?}",
            ids
        );
    }
...
"#,
        ...
        assert!(
            ids.contains(&"F1020".to_string()),
            "Expected F1020 for Ref to unknown target, got: {:?}",
            ids
        );
    }
...
"#,
        ...
        assert_eq!(
            f3012_count, 0,
            "String '443' should be coerced, not rejected — got {} F3012s",
            f3012_count
        );
    }
...
"#,
        ...
        let f3012_port = ds
            .iter()
            .any(|(id, msg)| id == "F3012" && (msg.contains("FromPort") || msg.contains("ToPort")));
        assert!(
            !f3012_port,
            "Native integers should not trigger F3012 for FromPort/ToPort, got: {:?}",
            ds
        );
    }
...
"#,
        ...
        assert!(
            has_w3012,
            "Expected W9003 coercion warning for string 'true' on PropagateAtLaunch, got: {:?}",
            ids
        );
        let has_f3012 = ids.iter().any(|id| id == "F3012");
        assert!(
            !has_f3012,
            "String 'true' should be coerced, not rejected — got F3012 in: {:?}",
            ids
        );
    }
...
mod consistency_tests {
    ...
    fn list_rules_covers_all_expected_categories() {
        let engine = CelEngine::new(EngineConfig::default()).unwrap();
        let rules = engine.list_rules();
        let categories: std::collections::HashSet<&str> = rules
            .iter()
            .map(|r| r.category.as_deref().unwrap_or(""))
            .collect();
        ...
        for expected in [
            ...
            assert!(
                ...
                "list_rules missing category '{}', got: {:?}",
                expected,
                categories
            );
        }
...
mod rule_category_tests {
    ...
    fn structure_duplicate_keys() {
        ...
        assert!(
            has_rule(&ids, "F0000"),
            "Expected E0000 for duplicate keys, got: {:?}",
            ids
        );
    }
    ...
    fn structure_not_cloudformation() {
        ...
        assert!(
            has_rule(&ids, "F0001") || has_rule(&ids, "F0000"),
            "Expected structure error, got: {:?}",
            ids
        );
    }
    ...
    fn intrinsics_bad_ref() {
        ...
        assert!(
            has_rule(&ids, "F1020"),
            "Expected F1020 for refs to unknown targets, got: {:?}",
            ids
        );
    }
    ...
    fn intrinsics_bad_findinmap() {
        ...
        assert!(
            has_rule(&ids, "F1012"),
            "Expected E1012 for FindInMap error, got: {:?}",
            ids
        );
    }
    ...
    fn intrinsics_bad_select() {
        ...
        assert!(
            has_select_warning,
            "Expected W1102 Select type warning, got: {:?}",
            ids
        );
    }
    ...
    fn intrinsics_bad_sub_needed() {
        ...
        assert!(
            has_rule(&ids, "F1029"),
            "Expected E1029 for Sub needed, got: {:?}",
            ids
        );
    }
    ...
    fn references_circular_dependency() {
        ...
        assert!(
            has_rule(&ids, "F3004"),
            "Expected E3004 for circular dependency, got: {:?}",
            ids
        );
    }
    ...
    fn references_circular_dependency_dependson() {
        ...
        assert!(
            has_rule(&ids, "F3004"),
            "Expected E3004 for DependsOn circular, got: {:?}",
            ids
        );
    }
    ...
    fn best_practices_deletion_policy() {
        ...
        assert!(
            has_deletion,
            "Expected deletion policy warning, got: {:?}",
            ids
        );
    }
    ...
    fn best_practices_hardcoded_arn() {
        ...
        assert!(
            has_rule(&ids, "I3042"),
            "Expected I3042 for hardcoded ARN, got: {:?}",
            ids
        );
    }
    ...
    fn best_practices_previous_gen_instance() {
        ...
        assert!(
            has_rule(&ids, "I3100"),
            "Expected I3100 for previous gen instance, got: {:?}",
            ids
        );
    }
    ...
    fn best_practices_good_deletion_policies_has_expected_warnings() {
        ...
        assert!(
            !ids.iter()
                .any(|id| id.starts_with("E") || id.starts_with("F")),
    ...
    fn resources_fargate_bad_cpu_memory() {
        ...
        assert!(
            has_rule(&ids, "E3047"),
            "Expected E3047 for Fargate CPU/memory, got: {:?}",
            ids
        );
    }
    ...
    fn resources_sg_bad_port_range() {
        ...
        assert!(
            has_rule(&ids, "E9002"),
            "Expected E9002 for bad port range, got: {:?}",
            ids
        );
    }
    ...
    fn resources_sqs_fifo_no_suffix() {
        ...
        assert!(
            has_rule(&ids, "E3501") || has_rule(&ids, "E2504"),
            "Expected SQS FIFO error, got: {:?}",
            ids
        );
    }
    ...
    fn resources_stepfunctions_bad_start_at() {
        ...
        assert!(
            has_rule(&ids, "E3601"),
            "Expected E3601 for StepFunctions StartAt, got: {:?}",
            ids
        );
    }
    ...
    fn resources_lambda_zip_no_handler() {
        ...
        assert!(
            has_rule(&ids, "F3003"),
            "Expected F3003 for Lambda zip no handler, got: {:?}",
            ids
        );
    }
    ...
    fn resources_good_ecs_fargate_valid() {
        ...
        assert!(
            !has_rule(&ids, "E3042"),
            "No E3042 expected for valid Fargate, got: {:?}",
            ids
        );
    }
    ...
    fn conditions_equals_wrong_arity() {
        ...
        assert!(
            has_rule(&ids, "F0014") || has_rule(&ids, "W8001"),
            "Expected Equals arity error, got: {:?}",
            ids
        );
    }
    ...
    fn conditions_good_no_errors() {
        ...
        assert!(
            ...
            "No condition errors expected, got: {:?}",
            cond_errors
        );
    }
    ...
    fn generated_rules_deprecated_type() {
        ...
        assert!(
            has_rule(&ids, "W9009"),
            "Expected W9009 for deprecated type, got: {:?}",
            ids
        );
    }
    ...
    fn generated_rules_unique_items() {
        ...
        assert!(
            has_rule(&ids, "W9007") || has_rule(&ids, "I3037"),
            "Expected unique items error, got: {:?}",
            ids
        );
    }
    ...
    fn good_generic_no_errors() {
        ...
        let errors: Vec<_> = ids
            .iter()
            .filter(|id| id.starts_with("E") || id.starts_with("F"))
            .collect();
        assert!(
            errors.is_empty(),
            "No errors expected for good/generic.yaml, got: {:?}",
            errors
        );
    }
    ...
    fn good_both_forms_no_errors() {
        ...
        assert!(
            errors.is_empty(),
            "No errors expected for good/both_forms.yaml, got: {:?}",
            errors
        );
    }
...
"#,
    ...
    fn e6101_getatt_types_integration_template() {
        ...
        assert!(
            e6101_count >= 5,
            "Expected at least 5 E6101 in getatt-types.yaml, got {}: {:?}",
            e6101_count,
            ids
        );
    }
...
"#,
        ...
        assert!(
            ...
            "Expected E9004 for invalid GetAtt attribute, got: {:?}",
            ids
        );
    }
}
