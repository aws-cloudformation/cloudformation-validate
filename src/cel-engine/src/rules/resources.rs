...
use template_model::consts::{
    FIELD_CREATION_POLICY, FIELD_RESOURCE_TYPE, FIELD_RESOURCES, FIELD_UPDATE_POLICY,
    KEY_CREATION_POLICY, KEY_PROPERTIES, KEY_UPDATE_POLICY,
};
...
pub fn register(reg: &mut NativeRuleRegistry) {
    ...
    reg.add(
        rules::Category::Resource,
        crate::rules::resources_extra::eval_extra_resources,
    );
}
...
fn eval_resources(ctx: &EvalContext) -> Vec<Diagnostic> {
    ...
    for (i, b_name) in subnets.iter().enumerate() {
        ...
        let Some(serde_json::Value::String(b_cidr)) =
            resolve_concrete(m, b_name, "Properties.CidrBlock")
        else {
            continue;
        };
        ...
        for a_name in &subnets[..i] {
            ...
            let Some(serde_json::Value::String(a_cidr)) =
                resolve_concrete(m, a_name, "Properties.CidrBlock")
            else {
                continue;
            };
            ...
            if rtype.ends_with("::MODULE") {
                if res.get(FIELD_CREATION_POLICY).is_some_and(|v| !v.is_null()) {
                    out.push(make_resource_diagnostic(
                        "E5001",
                        &format!(
                            "CreationPolicy is not permitted within Module resource '{}'",
                            name
                        ),
                        m,
                        name,
                ...
                if res.get(FIELD_UPDATE_POLICY).is_some_and(|v| !v.is_null()) {
                    out.push(make_resource_diagnostic(
                        "E5001",
                        &format!(
                            "UpdatePolicy is not permitted within Module resource '{}'",
                            name
                        ),
                        m,
                        name,
