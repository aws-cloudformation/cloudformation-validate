    ...
    fn evaluate_rules(
        ...
        *self.region_holder.lock().unwrap_or_else(|e| e.into_inner()) =
        ...
        if needed_core.len() == CORE_PACKAGES.len() {
            match rego.eval_rule("data.all_violations.violation".to_string()) {
                Ok(val) => {
                    let json_str = val
                        .to_json_str()
                        .expect("core violation result must serialize to JSON");
                    extract_diagnostics(
                        &json_str,
                        model,
                ...
                Err(e) => {
                    ...
                    for pkg in &needed_core {
                        let val = rego.eval_rule(pkg.to_string()).unwrap_or_else(|e| {
                            panic!(
                                "Core rule package '{}' evaluation must not fail: {}",
                                pkg, e
                            )
                        });
                        let json_str = val
        ...
        for pkg in &self.custom_packages {
            match rego.eval_rule(pkg.clone()) {
                Ok(val) => {
                    let json_str = val
                        .to_json_str()
                        .expect("successful eval result must serialize to JSON");
                    extract_diagnostics(
                        &json_str,
                        model,
                        &self.registry_metadata,
                ...
                Err(e) => {
                    warn!("Custom rule package '{}' evaluation failed: {}", pkg, e);
                }
            }
        }
        ...
        for pkg in &self.guard_packages {
            match rego.eval_rule(pkg.clone()) {
                ...
                Err(e) => {
                    warn!("Guard rule package '{}' evaluation failed: {}", pkg, e);
                }
            }
        }
