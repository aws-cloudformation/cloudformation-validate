...
#[test]
fn e2e_bad_fargate_task_requirements() {
    let report = validate_fixture("bad/ecs_fargate_task_requirements.yaml");
    let flagged: Vec<&str> = report
        .diagnostics
        .iter()
        .filter(|d| d.rule_id == "E3048")
        .filter_map(|d| d.entity.as_ref().map(|e| e.logical_id.as_str()))
        .collect();
    for logical_id in [
        "MissingTaskSize",
        "WrongNetworkMode",
...
fn e2e_good_fargate_task_requirements() {
    ...
    for rule_id in ["E3048", "E3047"] {
        assert!(!has_rule(&report, rule_id), "{rule_id} must not fire, got: {:?}", report.diagnostics);
    }
}

#[test]
fn e2e_bad_resource_attributes() {
    let report = validate_fixture("bad/resources/resource_attributes.yaml");
    let flagged: Vec<&str> = report
        .diagnostics
        .iter()
        .filter(|d| d.rule_id == "E3001")
        .filter_map(|d| d.entity.as_ref().map(|e| e.logical_id.as_str()))
        .collect();
    for logical_id in [
        "UnknownAttribute",
        ...
        assert!(flagged.contains(&logical_id), "Expected E3001 on {logical_id}, got: {flagged:?}");
    }
}

#[test]
fn e2e_good_resource_attributes() {
    let report = validate_fixture("good/resources/resource_attributes.yaml");
    assert!(!has_rule(&report, "E3001"), "E3001 must not fire, got: {:?}", report.diagnostics);
}
