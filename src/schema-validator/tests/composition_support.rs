...
fn one_of_is_decided_per_condition_scenario() {
    ...
    let template = r#"{
        ...
        "Conditions": { "IsA": { "Fn::Equals": [{ "Ref": "Mode" }, "a"] } },
        "Resources": {
            "R": {
...
// --- requiredOr: static NoValue treated as absent ---

#[test]
fn required_or_static_novalue_treated_as_absent() {
    // A member whose value is AWS::NoValue (null) at deploy time is absent.
    // When ALL members resolve to null, the constraint fires.
    let sv = validator(vec![(
        "AWS::Test::RequiredOrNoValue",
        json!({
            "properties": {
                "A": { "type": "string" },
                "B": { "type": "string" },
    ...
    // Both candidate properties resolve to AWS::NoValue.
    let template = r#"{
        "Resources": {
            "R": {
                "Type": "AWS::Test::RequiredOrNoValue",
                "Properties": {
                    "A": { "Ref": "AWS::NoValue" },
                    "B": { "Ref": "AWS::NoValue" },
                    "C": "present"
    ...
    assert!(
        ...
        "both members null - expected F3058: {:?}",
        diags.iter().map(|d| (&d.rule_id, &d.message)).collect::<Vec<_>>()
    );
}

#[test]
fn required_or_one_concrete_member_is_valid() {
    // When at least one member has a concrete value, no violation.
    let sv = validator(vec![(
        "AWS::Test::RequiredOrNoValue",
        json!({
            "properties": {
    ...
    assert!(
        ...
        "one member present - should not fire F3058: {:?}",
        diags.iter().map(|d| (&d.rule_id, &d.message)).collect::<Vec<_>>()
    );
}

// --- requiredOr: conditional violation with condition_scenario attached ---

#[test]
fn required_or_conditional_violation_carries_scenario() {
    // Member A is present when IsA=true (via Fn::If) but absent (NoValue) when
    // IsA=false. Member B is always absent. The violation fires only for the
    // IsA=false scenario and carries that condition.
    let sv = validator(vec![(
        "AWS::Test::RequiredOrCond",
        json!({
            "properties": {
                "A": { "type": "string" },
                "B": { "type": "string" }
    ...
    let template = r#"{
        ...
        "Resources": {
            "R": {
                "Type": "AWS::Test::RequiredOrCond",
                "Properties": {
                    "A": { "Fn::If": ["IsA", "value", { "Ref": "AWS::NoValue" }] }
                }
            }
        }
    }"#;
    let diags = validate(&sv, template);
    let finding = diags.iter().find(|d| d.rule_id == "F3058");
    assert!(
        finding.is_some(),
        "expected F3058 for the IsA=false scenario: {:?}",
        diags.iter().map(|d| (&d.rule_id, &d.message)).collect::<Vec<_>>()
    );
    let conds =
        finding.unwrap().condition_scenario.as_ref().expect("finding must carry the violating condition assignment");
    assert_eq!(conds.get("IsA"), Some(&false), "the violation is the IsA=false branch: {conds:?}");
}

#[test]
fn required_or_valid_complement_no_diagnostic() {
    // Both branches of the condition supply at least one member -- no violation.
    let sv = validator(vec![(
        "AWS::Test::RequiredOrCond",
        json!({
            "properties": {
    ...
    let template = r#"{
        ...
        "Resources": {
            "R": {
                ...
                "Properties": {
                    "A": { "Fn::If": ["IsA", "value", { "Ref": "AWS::NoValue" }] },
                    "B": { "Fn::If": ["IsA", { "Ref": "AWS::NoValue" }, "value"] }
                }
            }
        }
    }"#;
    ...
    assert!(
        !mentions(&diags, "F3058", ""),
        "complementary branches each satisfy requiredOr - no violation: {:?}",
        diags.iter().map(|d| (&d.rule_id, &d.message)).collect::<Vec<_>>()
    );
}

// --- requiredXor: complementary branches are clean ---

#[test]
fn required_xor_complementary_branches_clean() {
    // A and B are complementary via Fn::If on the same condition: exactly one
    // is present in each scenario.
    let sv = validator(vec![(
        "AWS::Test::RequiredXorComp",
        json!({
            "properties": {
                "A": { "type": "string" },
                "B": { "type": "string" }
    ...
    let template = r#"{
        ...
        "Conditions": { "UseA": { "Fn::Equals": [{ "Ref": "Choice" }, "a"] } },
        "Resources": {
            "R": {
                "Type": "AWS::Test::RequiredXorComp",
                "Properties": {
                    "A": { "Fn::If": ["UseA", "val", { "Ref": "AWS::NoValue" }] },
                    "B": { "Fn::If": ["UseA", { "Ref": "AWS::NoValue" }, "val"] }
                }
            }
        }
    }"#;
    ...
    assert!(
        ...
        "complementary xor branches must be clean: {:?}",
        diags.iter().map(|d| (&d.rule_id, &d.message)).collect::<Vec<_>>()
    );
}

// --- requiredXor: independent conditions -- both-present and none-present ---

#[test]
fn required_xor_independent_conditions_both_present_violation() {
    // A depends on CondA, B depends on CondB. When both conditions are true,
    // both members are present -- violating exactly-one.
    let sv = validator(vec![(
        "AWS::Test::RequiredXorIndep",
        json!({
            "properties": {
                "A": { "type": "string" },
                "B": { "type": "string" }
    ...
    let template = r#"{
        "Parameters": { "P1": { "Type": "String" }, "P2": { "Type": "String" } },
        "Conditions": {
            "CondA": { "Fn::Equals": [{ "Ref": "P1" }, "yes"] },
            "CondB": { "Fn::Equals": [{ "Ref": "P2" }, "yes"] }
        },
        "Resources": {
            "R": {
                "Type": "AWS::Test::RequiredXorIndep",
                "Properties": {
                    "A": { "Fn::If": ["CondA", "valA", { "Ref": "AWS::NoValue" }] },
                    "B": { "Fn::If": ["CondB", "valB", { "Ref": "AWS::NoValue" }] }
                }
            }
        }
    }"#;
    let diags = validate(&sv, template);
    let f3014 = diags.iter().filter(|d| d.rule_id == "F3014").collect::<Vec<_>>();
    let both_present_violation = f3014.iter().any(|d| {
        d.condition_scenario
            .as_ref()
            .is_some_and(|scenario| scenario.get("CondA") == Some(&true) && scenario.get("CondB") == Some(&true))
    });
    assert!(both_present_violation, "expected F3014 for the scenario where both conditions are true: {:?}", f3014);
}

#[test]
fn required_xor_independent_conditions_none_present_violation() {
    // Both A and B are NoValue when their respective conditions are false.
    // The scenario where both conditions are false has zero present -- violation.
    let sv = validator(vec![(
        "AWS::Test::RequiredXorIndep",
        json!({
            "properties": {
    ...
    let diags = validate(&sv, template);
    let f3014 = diags.iter().filter(|d| d.rule_id == "F3014").collect::<Vec<_>>();
    // At least one violation for the none-present scenario
    let none_present_violation = f3014.iter().any(|d| {
        d.condition_scenario
            .as_ref()
            .is_some_and(|cs| cs.get("CondA") == Some(&false) && cs.get("CondB") == Some(&false))
    });
    assert!(none_present_violation, "expected F3014 for the scenario where both conditions are false: {:?}", f3014);
}

// --- requiredXor in nested property schemas ---

#[test]
fn required_xor_nested_property_schema() {
    let sv = validator(vec![(
        "AWS::Test::NestedXor",
        json!({
            "properties": {
    ...
    let valid = r#"{
        ...
        "Resources": {
            "R": {
                "Type": "AWS::Test::NestedXor",
                "Properties": {
                    "Config": {
                        "ModeA": { "Fn::If": ["UseA", "a", { "Ref": "AWS::NoValue" }] },
                        "ModeB": { "Fn::If": ["UseA", { "Ref": "AWS::NoValue" }, "b"] }
                    }
                }
            }
        }
    }"#;
    let diags = validate(&sv, valid);
    assert!(
        !mentions(&diags, "F3014", ""),
        "nested complementary branches must satisfy requiredXor: {:?}",
        diags.iter().map(|d| (&d.rule_id, &d.message)).collect::<Vec<_>>()
    );
    ...
    let invalid = r#"{
        "Resources": {
            "R": {
                "Type": "AWS::Test::NestedXor",
                "Properties": { "Config": { "ModeA": "a", "ModeB": "b" } }
            }
        }
    }"#;
    ...
    assert!(
        ...
        "nested requiredXor with both members present must fail: {:?}",
        diags.iter().map(|d| (&d.rule_id, &d.message)).collect::<Vec<_>>()
    );
}

// --- requiredOr inside a composition ref branch (validate_sub) ---

#[test]
fn composition_branch_required_or_via_definition() {
    let sv = validator(vec![(
        "AWS::Test::CompBranchOrRef",
        json!({
            "properties": {
                "A": { "type": "string" },
                "B": { "type": "string" },
            ...
            "definitions": {
                "NeedsAOrB": { "requiredOr": ["A", "B"] },
                "NeedsC": { "required": ["C"] }
            },
            "anyOf": [
                { "$ref": "#/definitions/NeedsAOrB" },
                { "$ref": "#/definitions/NeedsC" }
            ],
            "additionalProperties": false
        }),
    )]);
    ...
    assert!(
        mentions(&diags, "F3017", "not valid under any"),
        "requiredOr must participate in referenced branch validity: {:?}",
        diags.iter().map(|d| (&d.rule_id, &d.message)).collect::<Vec<_>>()
    );
    ...
    assert!(
        !mentions(&diags, "F3017", ""),
        "A must satisfy the referenced requiredOr branch: {:?}",
        diags.iter().map(|d| (&d.rule_id, &d.message)).collect::<Vec<_>>()
    );
}
