...
`expected/all_templates.json` is the recorded `cfn-validate --format detailed` output for the whole template corpus,
under both the rego and cel engines. Regenerate it with the `generate_golden` example, which builds the release
...
| `golden_file()`         | Path to `expected/all_templates.json`                                       |
