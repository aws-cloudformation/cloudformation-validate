...
impl EngineType {
    ...
    pub fn parse(raw: &str) -> Self {
        match raw.to_lowercase().as_str() {
            "rego" => EngineType::Rego,
            "cel" => EngineType::Cel,
            other => panic!("Unknown engine type '{}', must be 'rego' or 'cel'", other),
        }
    }
}
...
"#;
    ...
    fn engine_type_parse_accepts_case_insensitive_input() {
        assert_eq!(EngineType::parse("rego"), EngineType::Rego);
        assert_eq!(EngineType::parse("REGO"), EngineType::Rego);
        assert_eq!(EngineType::parse("Cel"), EngineType::Cel);
        assert_eq!(EngineType::parse("CEL"), EngineType::Cel);
    }

    #[test]
    #[should_panic(expected = "Unknown engine type 'unknown'")]
    fn engine_type_parse_panics_on_unknown_value() {
        EngineType::parse("unknown");
    }
}
