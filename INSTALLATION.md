...
# JVM binding (Kotlin/Java) -> bindings-jvm/generated/*.jar
...
artifacts (`cloudformation-validate-wasm.zip`,
`cloudformation-validate-jvm-*.jar`) plus, for each one, a detached signature
...
# openssl dgst -sha256 -verify signing-key.pem -signature cloudformation-validate-wasm.zip.sig cloudformation-validate-wasm.zip
```
