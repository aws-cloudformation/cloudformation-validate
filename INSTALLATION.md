...
| `cargo-about`                   | 0.9.0      | third-party license generation (binding builds) | `cargo install`                                                  |
...
Python 3.10+ is required for `scripts/generate_licenses.py` (invoked by the binding builds) and the other developer
scripts under `scripts/`. No third-party packages are needed.
...
Each GitHub release attaches the prebuilt bindings as signed artifacts:

- `cloudformation-validate.jar` — the JVM (Kotlin/Java) binding
- `cloudformation-validate.zip` — the Node.js (WASM) binding package

The CLI is not published as a prebuilt binary; build it from source as shown above.
